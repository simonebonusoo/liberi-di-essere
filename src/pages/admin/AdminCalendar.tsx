            ...
            >
              ...
              <div className="flex-1 border-l border-brand-100 pl-4">
                ...
                <p className="text-sm text-brand-500">
                  {a.client?.full_name || a.client?.email} · {a.staff?.full_name} · {a.location?.name ?? 'Sede'}
                </p>
              </div>
      ...
      <Modal open={Boolean(detail)} onClose={() => setDetail(null)} title="Dettaglio appuntamento">
        {detail && (
          <div className="space-y-4">
            ...
            <DetailRow label="Sede" value={detail.location?.name ?? '—'} />
...
function CreateAppointmentModal({
  ...
  const submit = async () => {
    if (!clientId || !serviceId || !staffId || !locationId) {
